...
export const Register: React.FC<RegisterProps> = ({ onSwitchToLogin, onRegisterSuccess }) => {
  ...
  const validateForm = (): boolean => {
    if (!firstName || !lastNamePaternal || !lastNameMaternal || !password || !passwordConfirmation) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const fullName = `${firstName} ${lastNamePaternal} ${lastNameMaternal}`.trim();
    const tempEmail = `${firstName.toLowerCase()}.${lastNamePaternal.toLowerCase()}@temp.com`;
    const profession = "Estudiante";

    try {
      await register(fullName, tempEmail, profession, password);
      onRegisterSuccess();
    } catch (err: any) {
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-100">
      <div className="max-w-md w-full bg-white rounded-lg shadow-lg p-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {/* Campo Nombre - label alineado a la izquierda */}
          ...
          {/* Campo Apellido Paterno - label alineado a la izquierda */}
          <div className="text-left">
            <label className="block text-sm font-medium text-gray-700 mb-1">
              Apellido Paterno
            </label>
          ...
          {/* Campo Apellido Materno - label alineado a la izquierda */}
          <div className="text-left">
            <label className="block text-sm font-medium text-gray-700 mb-1">
              Apellido Materno
            </label>
          ...
          {/* Campo Contraseña - label alineado a la izquierda */}
          <div className="text-left">
            <label className="block text-sm font-medium text-gray-700 mb-1">
              Contraseña
            </label>
          ...
          {/* Campo Confirmar Contraseña - label alineado a la izquierda */}
          <div className="text-left">
            <label className="block text-sm font-medium text-gray-700 mb-1">
        ...
        >
          ...
          <span className="text-sm text-gray-600">Iniciar sesión con Google</span>
        </button>

        {/* Enlace para ir a login - centrado */}
