...
// ── Formateador de fecha local (Bolivia UTC-4) ──────────────────────────────────
const formatLocalDateTime = (utcDate: string | null): string => {
  if (!utcDate || utcDate === 'Nunca') return 'Nunca';
  try {
...
const formatLocalDate = (utcDate: string | null): string => {
  if (!utcDate || utcDate === 'Nunca') return utcDate || 'Nunca';
  try {
    const date = new Date(utcDate);
    if (isNaN(date.getTime())) return utcDate;
...
const TABS: { key: Tab; label: string; desc: string }[] = [
  { key: 'usuarios',    label: 'Usuarios',     desc: 'Gestión de cuentas' },
  { key: 'bitacora',    label: 'Bitácora',     desc: 'Historial de acciones' },
  { key: 'tecnologias', label: 'Tecnologías',  desc: 'Catálogo de tecnologías' },
  { key: 'grados',      label: 'Grados',       desc: 'Grados académicos' },
  { key: 'anuncios',    label: 'Anuncios',     desc: 'Publicaciones' },
  { key: 'backup',      label: 'Backup',       desc: 'Importar / Exportar' },
];
...
const AdminUsuarios: React.FC = () => {
  ...
  const cargarBitacora = useCallback(async (pagina = 1) => {
    ...
    } catch { showToast('Error al cargar bitácora', 'error'); }
    finally { setLoadingB(false); }
  }, [filtrosB]);

  const cargarTecnologias = async () => {
    setLoadingT(true);
    try { const r = await getTecnologias(); setTecnologias(r.tecnologias); }
    catch { showToast('Error al cargar tecnologías', 'error'); }
    finally { setLoadingT(false); }
  };

  const cargarGrados = async () => {
    setLoadingG(true);
    try { const r = await getGrados(); setGrados(r.grados); }
    catch { showToast('Error al cargar grados', 'error'); }
    finally { setLoadingG(false); }
  };

  const cargarAnuncios = async () => {
    setLoadingAN(true);
    try { const r = await getAnuncios(); setAnuncios(r.anuncios); }
    catch { showToast('Error al cargar anuncios', 'error'); }
    finally { setLoadingAN(false); }
  };

  useEffect(() => { cargarUsuarios(1); }, [filtrosU]);
  useEffect(() => { if (tab === 'bitacora')    cargarBitacora(1);  }, [filtrosB, tab]);
  useEffect(() => { if (tab === 'tecnologias') cargarTecnologias(); }, [tab]);
  useEffect(() => { if (tab === 'grados')      cargarGrados();      }, [tab]);
  useEffect(() => { if (tab === 'anuncios')    cargarAnuncios();    }, [tab]);
  ...
  const handleSuspender = async () => {
    ...
    setSaving(true);
    try { await suspenderUsuario(modal.usuario.id_usuario); showToast('Cuenta suspendida', 'success'); cargarUsuarios(paginaU); }
  ...
  const handleReactivar = async () => {
    ...
    setSaving(true);
    try { await reactivarUsuario(modal.usuario.id_usuario); showToast('Cuenta reactivada', 'success'); cargarUsuarios(paginaU); }
  ...
  const handleDescargar = async () => {
    setDescargando(true);
    try { await descargarBackup(); showToast('Backup descargado', 'success'); }
  ...
  const handleImportar = async () => {
    ...
    try {
      ...
      setTimeout(() => { localStorage.clear(); navigate('/login', { replace: true, state: { message: 'Backup importado. Inicia sesión nuevamente.' } }); }, 2000);
    } catch { showToast('Error al importar', 'error'); setImportando(false); setModal(null); }
  };

  // ── Tecnologías ───────────────────────────────────────────────────────────
  const handleGuardarTec = async () => {
    if (!formTec.nombre.trim() || !formTec.categoria.trim()) { showToast('Nombre y categoría son obligatorios', 'error'); return; }
    setSaving(true);
    try {
      if (modal?.tipo === 'nueva-tecnologia') { await crearTecnologia(formTec); showToast('Tecnología creada', 'success'); }
      else if (modal?.tipo === 'editar-tecnologia') { await modificarTecnologia(modal.item.id_tecnologia, formTec); showToast('Tecnología actualizada', 'success'); }
      cargarTecnologias();
  ...
  const handleEliminarTec = async () => {
    ...
    setSaving(true);
    try { await eliminarTecnologia(modal.item.id_tecnologia); showToast('Tecnología eliminada', 'success'); cargarTecnologias(); }
  ...
  const handleGuardarGrado = async () => {
    if (!formGrado.nombre_grado.trim()) { showToast('El nombre es obligatorio', 'error'); return; }
    setSaving(true);
    try {
      if (modal?.tipo === 'nuevo-grado') { await crearGrado(formGrado); showToast('Grado creado', 'success'); }
      else if (modal?.tipo === 'editar-grado') { await modificarGrado(modal.item.id_grado, formGrado); showToast('Grado actualizado', 'success'); }
      cargarGrados();
    } catch { showToast('Error al guardar', 'error'); }
    finally { setSaving(false); setModal(null); }
  };
  ...
  const handleEliminarGrado = async () => {
    ...
    setSaving(true);
    try { await eliminarGrado(modal.item.id_grado); showToast('Grado eliminado', 'success'); cargarGrados(); }
    catch { showToast('Error al eliminar', 'error'); }
    finally { setSaving(false); setModal(null); }
  };

  // ── Anuncios ──────────────────────────────────────────────────────────────
  const resetFormAnuncio = () => { setFormAnuncio({ titulo: '', descripcion: '', url_redireccion: '' }); setFotoAnuncio(null); setPreviewFoto(null); };
  ...
  const handleGuardarAnuncio = async () => {
    if (!formAnuncio.titulo.trim() || !formAnuncio.url_redireccion.trim()) { showToast('Título y URL son obligatorios', 'error'); return; }
    if (modal?.tipo === 'nuevo-anuncio' && !formAnuncio.descripcion.trim() && !fotoAnuncio) { showToast('Incluye al menos descripción o imagen', 'error'); return; }
    setSaving(true);
    try {
    ...
    finally { setSaving(false); setModal(null); resetFormAnuncio(); }
  };

  const handleEliminarAnuncio = async () => {
    if (!modal || modal.tipo !== 'eliminar-anuncio') return;
    setSaving(true);
    try { await eliminarAnuncio(modal.item.id_anuncio); showToast('Anuncio eliminado', 'success'); cargarAnuncios(); }
    catch { showToast('Error al eliminar', 'error'); }
    finally { setSaving(false); setModal(null); }
  };
  ...
  const Modal = ({ children, onClose }: { children: React.ReactNode; onClose: () => void }) => (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-2xl p-7 w-full max-w-md max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        {children}
      </div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <div className="flex flex-1">
        ...
        {/* SIDEBAR */}
        <aside className="hidden lg:flex flex-col w-40 bg-app-sidebar text-white py-6 gap-2 items-stretch border-r border-app-border shrink-0">
          {TABS.map((t) => (
            <button
              key={t.key}
              onClick={() => setTab(t.key)}
              title={t.desc}
              className={`flex flex-col items-center justify-center px-3 py-3 rounded-xl gap-1 transition text-sm font-medium text-center mx-2
                ${tab === t.key ? 'bg-white/20 text-white' : 'bg-white/10 text-white/70 hover:bg-white/15 hover:text-white'}`}
            >
              <span className="text-sm font-medium">{t.label}</span>
              <span className="text-[10px] text-white/50">{t.desc}</span>
            </button>
          ))}
        </aside>
            ...
            <div className="space-y-5">
              ...
              <div className="flex gap-3 flex-wrap">
                <div className="flex items-center gap-2 flex-1 min-w-52 rounded-2xl border border-app-border bg-white px-4 py-2 shadow-sm">
                  ...
                  <input type="text" placeholder="Buscar por nombre o correo..." className="w-full bg-transparent text-sm outline-none text-gray-700 placeholder-gray-400" onChange={(e) => handleSearch(e.target.value)} />
                </div>
                <select className="rounded-2xl border border-app-border bg-white px-4 py-2 text-sm text-gray-700 shadow-sm outline-none"
              ...
              <div className="rounded-2xl border border-app-border bg-white shadow-sm overflow-hidden">
                {loadingU ? <SkeletonRows n={5} /> : usuarios.length === 0 ? (
                  <div className="flex flex-col items-center gap-2 py-12 text-gray-400"><span className="text-3xl">👥</span><p className="text-sm">No se encontraron usuarios</p></div>
                ) : (
                      ...
                      <tbody>
                        {usuarios.map((u) => (
                          <tr key={u.id_usuario} className={`border-b border-gray-50 hover:bg-gray-50 transition ${u.estado === 'suspendido' ? 'bg-red-50' : ''}`}>
                            ...
                            <td className="px-4 py-3 text-xs text-gray-400 whitespace-nowrap">{formatLocalDate(u.fecha_registro)}</td>
                            <td className="px-4 py-3 text-xs text-gray-400 whitespace-nowrap">{formatLocalDateTime(u.fecha_ult_acceso)}</td>
                            <td className="px-4 py-3">
                              {u.rol !== 'admin' && (
                                u.estado === 'activo' ? (
                                  <button onClick={() => setModal({ tipo: 'suspender', usuario: u })}
                                  ...
                                  <button onClick={() => setModal({ tipo: 'reactivar', usuario: u })}
                ...
                {!loadingU && lastPageU > 1 && (
                  <div className="flex items-center justify-between px-4 py-3 border-t border-gray-100">
                    <span className="text-xs text-gray-400">Página {paginaU} de {lastPageU} — {totalU} usuarios</span>
                    <div className="flex gap-2">
                      <button className="rounded-full border border-gray-200 px-3 py-1 text-xs disabled:opacity-40" disabled={paginaU === 1} onClick={() => cargarUsuarios(paginaU - 1)}>← Anterior</button>
                      <button className="rounded-full border border-gray-200 px-3 py-1 text-xs disabled:opacity-40" disabled={paginaU === lastPageU} onClick={() => cargarUsuarios(paginaU + 1)}>Siguiente →</button>
                    </div>
                  </div>
                )}
              </div>
            ...
            <div className="space-y-5">
              ...
              <div className="flex gap-3 flex-wrap items-end">
                <div className="flex flex-col gap-1">
                  ...
                  <input type="date" className="rounded-xl border border-app-border bg-white px-3 py-2 text-sm outline-none"
                ...
                <select className="rounded-xl border border-app-border bg-white px-3 py-2 text-sm outline-none"
                ...
                <button className="rounded-xl border border-app-border bg-white px-4 py-2 text-sm hover:bg-gray-50 transition" onClick={() => cargarBitacora(1)}>
                   Aplicar
                </button>
              </div>

              <div className="rounded-2xl border border-app-border bg-white shadow-sm overflow-hidden">
                {loadingB ? <SkeletonRows n={4} /> : bitacora.length === 0 ? (
                  <div className="flex flex-col items-center gap-2 py-12 text-gray-400"><span className="text-3xl">📋</span><p className="text-sm">No hay registros</p></div>
                ) : (
                  <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                      <thead className="border-b border-gray-100">
                        <tr>
                          {['Usuario', 'Tipo de acción', 'Estado actual', 'Fecha y hora'].map((h) => (
                      ...
                      <tbody>
                        {bitacora.map((b) => (
                          <tr key={b.id_registro} className="border-b border-gray-50 hover:bg-gray-50 transition">
                            <td className="px-4 py-3">
                              <div className="flex items-center gap-3">
                                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold shrink-0 ${b.estado_actual === 'suspendido' ? 'bg-red-100 text-red-700' : 'bg-blue-100 text-blue-700'}`}>
                                  {iniciales(b.nombre)}
                                </div>
                            ...
                            <td className="px-4 py-3 text-xs text-gray-400 whitespace-nowrap">{formatLocalDateTime(b.fecha_accion)}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                ...
                {!loadingB && lastPageB > 1 && (
                  <div className="flex items-center justify-between px-4 py-3 border-t border-gray-100">
                    <span className="text-xs text-gray-400">Página {paginaB} de {lastPageB} — {totalBit} registros</span>
                    <div className="flex gap-2">
                      <button className="rounded-full border border-gray-200 px-3 py-1 text-xs disabled:opacity-40" disabled={paginaB === 1} onClick={() => cargarBitacora(paginaB - 1)}>← Anterior</button>
                      <button className="rounded-full border border-gray-200 px-3 py-1 text-xs disabled:opacity-40" disabled={paginaB === lastPageB} onClick={() => cargarBitacora(paginaB + 1)}>Siguiente →</button>
                    </div>
                  </div>
                )}
              </div>
          ...
          {tab === 'tecnologias' && (
            <div className="space-y-5">
              <div className="flex items-start justify-between">
                ...
                <button onClick={() => { setFormTec({ nombre: '', categoria: '' }); setModal({ tipo: 'nueva-tecnologia' }); }}
              ...
              <div className="rounded-2xl border border-app-border bg-white shadow-sm overflow-hidden">
                {loadingT ? <SkeletonRows n={4} /> : tecnologias.length === 0 ? (
                  <div className="flex flex-col items-center gap-2 py-12 text-gray-400"><span className="text-3xl">⚙️</span><p className="text-sm">No hay tecnologías</p></div>
                ) : (
                  <table className="w-full text-sm">
                    <thead className="border-b border-gray-100">
                      <tr>
                        {['Nombre', 'Categoría', 'Acciones'].map((h) => (
                          <th key={h} className="px-4 py-3 text-left text-xs font-semibold text-gray-400 uppercase tracking-wide">{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                      {tecnologias.map((t) => (
                        <tr key={t.id_tecnologia} className="border-b border-gray-50 hover:bg-gray-50 transition">
                          ...
                          <td className="px-4 py-3"><span className="text-xs bg-gray-100 text-gray-600 px-2.5 py-1 rounded-full">{t.categoria}</span></td>
                          <td className="px-4 py-3">
                            <div className="flex gap-2">
                              <button onClick={() => { setFormTec({ nombre: t.nombre, categoria: t.categoria }); setModal({ tipo: 'editar-tecnologia', item: t }); }}
                                className="rounded-full border border-gray-200 px-3 py-1 text-xs hover:bg-gray-50 transition">Editar</button>
                              <button onClick={() => setModal({ tipo: 'eliminar-tecnologia', item: t })}
                                className="rounded-full border border-red-200 text-red-600 px-3 py-1 text-xs hover:bg-red-50 transition">Eliminar</button>
                            </div>
                          </td>
                        </tr>
                      ))}
          ...
          {tab === 'grados' && (
            <div className="space-y-5">
              <div className="flex items-start justify-between">
                ...
                <button onClick={() => { setFormGrado({ nombre_grado: '' }); setModal({ tipo: 'nuevo-grado' }); }}
                  className="rounded-full bg-blue-600 text-white px-4 py-2 text-sm font-medium hover:bg-blue-700 transition">
                  + Agregar grado
                </button>
              </div>
              ...
              <div className="rounded-2xl border border-app-border bg-white shadow-sm overflow-hidden">
                {loadingG ? <SkeletonRows n={4} /> : grados.length === 0 ? (
                  <div className="flex flex-col items-center gap-2 py-12 text-gray-400"><span className="text-3xl">🎓</span><p className="text-sm">No hay grados</p></div>
                ) : (
                  <table className="w-full text-sm">
                    <thead className="border-b border-gray-100">
                      <tr>
                        {['Nombre del grado', 'Acciones'].map((h) => (
                          <th key={h} className="px-4 py-3 text-left text-xs font-semibold text-gray-400 uppercase tracking-wide">{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                      {grados.map((g) => (
                        <tr key={g.id_grado} className="border-b border-gray-50 hover:bg-gray-50 transition">
                          ...
                          <td className="px-4 py-3">
                            <div className="flex gap-2">
                              <button onClick={() => { setFormGrado({ nombre_grado: g.nombre_grado }); setModal({ tipo: 'editar-grado', item: g }); }}
                                className="rounded-full border border-gray-200 px-3 py-1 text-xs hover:bg-gray-50 transition">Editar</button>
                              <button onClick={() => setModal({ tipo: 'eliminar-grado', item: g })}
                                className="rounded-full border border-red-200 text-red-600 px-3 py-1 text-xs hover:bg-red-50 transition">Eliminar</button>
                            </div>
                          </td>
                        </tr>
          ...
          {tab === 'anuncios' && (
            <div className="space-y-5">
              <div className="flex items-start justify-between">
                ...
                <button onClick={() => { resetFormAnuncio(); setModal({ tipo: 'nuevo-anuncio' }); }}
                  className="rounded-full bg-blue-600 text-white px-4 py-2 text-sm font-medium hover:bg-blue-700 transition">
                  + Agregar anuncio
                </button>
              </div>

              {loadingAN ? <SkeletonRows n={3} /> : anuncios.length === 0 ? (
                <div className="rounded-2xl border border-app-border bg-white shadow-sm flex flex-col items-center gap-2 py-12 text-gray-400">
                  <span className="text-3xl">📢</span><p className="text-sm">No hay anuncios</p>
                </div>
              ) : (
                  ...
                  {anuncios.map((a) => (
                    <article key={a.id_anuncio} className="rounded-2xl border border-app-border bg-white shadow-sm overflow-hidden flex flex-col">
                      ...
                      <div className="p-4 flex flex-col gap-2 flex-1">
                        ...
                        <p className="text-xs text-gray-400">{formatLocalDateTime(a.creado_en)}</p>
                      </div>
                      <div className="flex gap-2 px-4 py-3 border-t border-gray-100">
                        <button onClick={() => { setFormAnuncio({ titulo: a.titulo, descripcion: a.descripcion ?? '', url_redireccion: a.url_redireccion }); setPreviewFoto(a.foto_url); setFotoAnuncio(null); setModal({ tipo: 'editar-anuncio', item: a }); }}
                          className="flex-1 rounded-full border border-gray-200 px-3 py-1.5 text-xs hover:bg-gray-50 transition">Editar</button>
                        <button onClick={() => setModal({ tipo: 'eliminar-anuncio', item: a })}
                          className="flex-1 rounded-full border border-red-200 text-red-600 px-3 py-1.5 text-xs hover:bg-red-50 transition">Eliminar</button>
                      </div>
                    </article>
                  ))}
                </div>
      ...
      {modal?.tipo === 'suspender' && (
        <Modal onClose={() => setModal(null)}>
          ...
          <div className="flex items-center gap-3 bg-gray-50 rounded-xl p-3 mb-5">
            <div className="w-8 h-8 rounded-full bg-red-100 text-red-700 flex items-center justify-center text-xs font-bold shrink-0">{iniciales(modal.usuario.nombre)}</div>
          ...
          <div className="flex gap-3">
            <button className="flex-1 border border-gray-200 rounded-full py-2 text-sm hover:bg-gray-50" onClick={() => setModal(null)} disabled={saving}>Cancelar</button>
            <button className="flex-1 bg-red-600 text-white rounded-full py-2 text-sm font-medium hover:bg-red-700 disabled:opacity-50" onClick={handleSuspender} disabled={saving}>{saving ? 'Procesando...' : 'Suspender'}</button>
          </div>
        </Modal>
      )}
      ...
      {modal?.tipo === 'reactivar' && (
        <Modal onClose={() => setModal(null)}>
          ...
          <div className="flex items-center gap-3 bg-gray-50 rounded-xl p-3 mb-5">
            <div className="w-8 h-8 rounded-full bg-blue-100 text-blue-700 flex items-center justify-center text-xs font-bold shrink-0">{iniciales(modal.usuario.nombre)}</div>
            <div><p className="text-sm font-medium text-gray-800">{modal.usuario.nombre}</p><p className="text-xs text-gray-400">{modal.usuario.email}</p></div>
          </div>
          <div className="flex gap-3">
            <button className="flex-1 border border-gray-200 rounded-full py-2 text-sm hover:bg-gray-50" onClick={() => setModal(null)} disabled={saving}>Cancelar</button>
            <button className="flex-1 bg-green-600 text-white rounded-full py-2 text-sm font-medium hover:bg-green-700 disabled:opacity-50" onClick={handleReactivar} disabled={saving}>{saving ? 'Procesando...' : 'Reactivar'}</button>
          </div>
        </Modal>
      )}
      ...
      {modal?.tipo === 'importar' && (
        <Modal onClose={() => !importando && setModal(null)}>
          ...
          <p className="text-sm text-gray-500 mb-3">Se importará <strong>{modal.archivo.name}</strong>. Esta acción sobreescribirá todos los datos y cerrará todas las sesiones.</p>
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 text-xs text-amber-700 mb-5">Esta acción es <strong>irreversible</strong>. Asegúrate de tener una copia de seguridad.</div>
          <div className="flex gap-3">
            <button className="flex-1 border border-gray-200 rounded-full py-2 text-sm hover:bg-gray-50" onClick={() => setModal(null)} disabled={importando}>Cancelar</button>
            <button className="flex-1 bg-red-600 text-white rounded-full py-2 text-sm font-medium hover:bg-red-700 disabled:opacity-50" onClick={handleImportar} disabled={importando}>{importando ? 'Importando...' : 'Confirmar'}</button>
          </div>
        </Modal>
      )}
      ...
      {(modal?.tipo === 'nueva-tecnologia' || modal?.tipo === 'editar-tecnologia') && (
        <Modal onClose={() => setModal(null)}>
          <h3 className="text-base font-semibold text-gray-800 mb-4">{modal.tipo === 'nueva-tecnologia' ? '+ Nueva tecnología' : 'Editar tecnología'}</h3>
          <div className="space-y-3 mb-5">
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1">Nombre *</label>
              <input className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm outline-none focus:border-blue-400" placeholder="Ej: React" value={formTec.nombre} onChange={(e) => setFormTec((p) => ({ ...p, nombre: e.target.value }))} />
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1">Categoría *</label>
              <input className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm outline-none focus:border-blue-400" placeholder="Ej: Frontend" value={formTec.categoria} onChange={(e) => setFormTec((p) => ({ ...p, categoria: e.target.value }))} />
            </div>
          </div>
          <div className="flex gap-3">
            <button className="flex-1 border border-gray-200 rounded-full py-2 text-sm hover:bg-gray-50" onClick={() => setModal(null)} disabled={saving}>Cancelar</button>
            <button className="flex-1 bg-blue-600 text-white rounded-full py-2 text-sm font-medium hover:bg-blue-700 disabled:opacity-50" onClick={handleGuardarTec} disabled={saving}>{saving ? 'Guardando...' : 'Guardar'}</button>
          </div>
        </Modal>
      )}
      ...
      {modal?.tipo === 'eliminar-tecnologia' && (
        <Modal onClose={() => setModal(null)}>
          ...
          <p className="text-sm text-gray-500 mb-5">¿Seguro que deseas eliminar <strong>{modal.item.nombre}</strong>? Se eliminarán también sus asociaciones con proyectos.</p>
          <div className="flex gap-3">
            <button className="flex-1 border border-gray-200 rounded-full py-2 text-sm hover:bg-gray-50" onClick={() => setModal(null)} disabled={saving}>Cancelar</button>
            <button className="flex-1 bg-red-600 text-white rounded-full py-2 text-sm font-medium hover:bg-red-700 disabled:opacity-50" onClick={handleEliminarTec} disabled={saving}>{saving ? 'Eliminando...' : 'Eliminar'}</button>
          </div>
        </Modal>
      )}
      ...
      {(modal?.tipo === 'nuevo-grado' || modal?.tipo === 'editar-grado') && (
        <Modal onClose={() => setModal(null)}>
          <h3 className="text-base font-semibold text-gray-800 mb-4">{modal.tipo === 'nuevo-grado' ? '+ Nuevo grado' : 'Editar grado'}</h3>
          <div className="mb-5">
            <label className="block text-xs font-medium text-gray-600 mb-1">Nombre del grado *</label>
            <input className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm outline-none focus:border-blue-400" placeholder="Ej: Licenciatura" value={formGrado.nombre_grado} onChange={(e) => setFormGrado({ nombre_grado: e.target.value })} />
          </div>
          <div className="flex gap-3">
            <button className="flex-1 border border-gray-200 rounded-full py-2 text-sm hover:bg-gray-50" onClick={() => setModal(null)} disabled={saving}>Cancelar</button>
            <button className="flex-1 bg-blue-600 text-white rounded-full py-2 text-sm font-medium hover:bg-blue-700 disabled:opacity-50" onClick={handleGuardarGrado} disabled={saving}>{saving ? 'Guardando...' : 'Guardar'}</button>
          </div>
        </Modal>
      )}
      ...
      {modal?.tipo === 'eliminar-grado' && (
        <Modal onClose={() => setModal(null)}>
          ...
          <p className="text-sm text-gray-500 mb-5">¿Seguro que deseas eliminar <strong>{modal.item.nombre_grado}</strong>?</p>
          <div className="flex gap-3">
            <button className="flex-1 border border-gray-200 rounded-full py-2 text-sm hover:bg-gray-50" onClick={() => setModal(null)} disabled={saving}>Cancelar</button>
            <button className="flex-1 bg-red-600 text-white rounded-full py-2 text-sm font-medium hover:bg-red-700 disabled:opacity-50" onClick={handleEliminarGrado} disabled={saving}>{saving ? 'Eliminando...' : 'Eliminar'}</button>
          </div>
        </Modal>
      )}

      {(modal?.tipo === 'nuevo-anuncio' || modal?.tipo === 'editar-anuncio') && (
        <Modal onClose={() => { setModal(null); resetFormAnuncio(); }}>
          <h3 className="text-base font-semibold text-gray-800 mb-4">{modal.tipo === 'nuevo-anuncio' ? '+ Nuevo anuncio' : 'Editar anuncio'}</h3>
          <div className="space-y-3 mb-5">
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1">Título *</label>
              <input className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm outline-none focus:border-blue-400" placeholder="Título del anuncio" value={formAnuncio.titulo} onChange={(e) => setFormAnuncio((p) => ({ ...p, titulo: e.target.value }))} />
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1">Descripción</label>
              <textarea className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm outline-none focus:border-blue-400 resize-none" rows={3} placeholder="Descripción (opcional si sube imagen)" value={formAnuncio.descripcion} onChange={(e) => setFormAnuncio((p) => ({ ...p, descripcion: e.target.value }))} />
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1">URL de redirección *</label>
              <input className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm outline-none focus:border-blue-400" placeholder="https://..." value={formAnuncio.url_redireccion} onChange={(e) => setFormAnuncio((p) => ({ ...p, url_redireccion: e.target.value }))} />
            </div>
            <div>
              ...
              <input ref={fotoInputRef} type="file" accept="image/jpg,image/jpeg,image/png,image/webp" className="hidden" onChange={handleFotoAnuncio} />
            </div>
          </div>
          <div className="flex gap-3">
            <button className="flex-1 border border-gray-200 rounded-full py-2 text-sm hover:bg-gray-50" onClick={() => { setModal(null); resetFormAnuncio(); }} disabled={saving}>Cancelar</button>
            <button className="flex-1 bg-blue-600 text-white rounded-full py-2 text-sm font-medium hover:bg-blue-700 disabled:opacity-50" onClick={handleGuardarAnuncio} disabled={saving}>{saving ? 'Guardando...' : 'Guardar'}</button>
          </div>
        </Modal>
      )}
      ...
      {modal?.tipo === 'eliminar-anuncio' && (
        <Modal onClose={() => setModal(null)}>
          ...
          <p className="text-sm text-gray-500 mb-5">¿Seguro que deseas eliminar <strong>{modal.item.titulo}</strong>? La imagen también será eliminada.</p>
          <div className="flex gap-3">
            <button className="flex-1 border border-gray-200 rounded-full py-2 text-sm hover:bg-gray-50" onClick={() => setModal(null)} disabled={saving}>Cancelar</button>
            <button className="flex-1 bg-red-600 text-white rounded-full py-2 text-sm font-medium hover:bg-red-700 disabled:opacity-50" onClick={handleEliminarAnuncio} disabled={saving}>{saving ? 'Eliminando...' : 'Eliminar'}</button>
          </div>
        </Modal>
      )}
      ...
      {/* TOAST */}
      {toast && (
        <div className={`fixed bottom-6 right-6 z-50 flex items-center gap-3 px-4 py-3 rounded-2xl text-sm font-medium shadow-lg transition-all
