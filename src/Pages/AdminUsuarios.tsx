...
const AdminUsuarios: React.FC = () => {
  ...
  const [errFechaDesde, setErrFechaDesde] = useState('');
  const [errFechaHasta, setErrFechaHasta] = useState('');
  ...
  const validarFecha = (valor: string): string => {
    if (!valor) return '';
    const d = new Date(valor);
    if (isNaN(d.getTime())) return 'Fecha inválida';
    if (valor < FECHA_MIN) return `Mínimo ${FECHA_MIN}`;
    if (valor > FECHA_MAX) return `Máximo ${FECHA_MAX}`;
    return '';
  };

  const handleFechaDesde = (valor: string) => {
    const err = validarFecha(valor);
    setErrFechaDesde(err);
    if (!err) setFiltrosB((p) => ({ ...p, fecha_desde: valor || undefined }));
  };

  const handleFechaHasta = (valor: string) => {
    const err = validarFecha(valor);
    setErrFechaHasta(err);
    if (!err) setFiltrosB((p) => ({ ...p, fecha_hasta: valor || undefined }));
  };
  ...
  const abrirEditarTec = (item: Tecnologia) => {
    setFormTec({ nombre: item.nombre, categoria: item.categoria });
    setModal({ tipo: 'editar-tecnologia', item });
  };
  ...
  const abrirEditarGrado = (item: Grado) => {
    setFormGrado({ nombre_grado: item.nombre_grado });
    setModal({ tipo: 'editar-grado', item });
  };
  ...
  const abrirEditarAnuncio = (item: Anuncio) => {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <header className="flex items-center justify-between bg-app-header px-6 py-4 text-white">
        ...
        <button onClick={handleLogout} className="flex items-center gap-2 rounded-full border border-white/20 bg-white/10 px-4 py-2 text-sm font-medium hover:bg-white/20 transition">
      ...
      <div className="flex flex-1">
        {/* SIDEBAR */}
        <aside className="hidden lg:flex flex-col w-20 bg-app-sidebar text-white py-6 gap-2 items-center border-r border-app-border shrink-0">
          {TABS.map((t) => (
            <button key={t.key} onClick={() => setTab(t.key)} title={t.label}
              className={`flex flex-col items-center justify-center w-14 h-14 rounded-xl gap-1 transition text-xs font-medium
            ...
            <div className="space-y-5">
              ...
              <div className="flex gap-3 flex-wrap items-end">
                <div className="flex flex-col gap-1">
                  ...
                  <input type="date" min={FECHA_MIN} max={FECHA_MAX} defaultValue=""
                    className={`rounded-xl border px-3 py-2 text-sm outline-none bg-white ${errFechaDesde ? 'border-red-400' : 'border-app-border'}`}
                    onChange={(e) => handleFechaDesde(e.target.value)} />
                  {errFechaDesde && <span className="text-xs text-red-500">{errFechaDesde}</span>}
                </div>
                <div className="flex flex-col gap-1">
                  <label className="text-xs text-gray-400">Hasta</label>
                  <input type="date" min={FECHA_MIN} max={FECHA_MAX} defaultValue=""
                    className={`rounded-xl border px-3 py-2 text-sm outline-none bg-white ${errFechaHasta ? 'border-red-400' : 'border-app-border'}`}
                    onChange={(e) => handleFechaHasta(e.target.value)} />
                  {errFechaHasta && <span className="text-xs text-red-500">{errFechaHasta}</span>}
                </div>
                      ...
                      <tbody>
                        {bitacora.map((b) => (
                          <tr key={b.id_registro} className="border-b border-gray-50 hover:bg-gray-50 transition">
                            ...
                            <td className="px-4 py-3">
                              <button onClick={() => setModalContexto(b)}
            ...
            <div className="space-y-5">
              ...
              <div className="rounded-2xl border border-app-border bg-white shadow-sm overflow-hidden">
                {loadingT ? <SkeletonRows n={4} /> : tecnologias.length === 0 ? (
                  <div className="flex flex-col items-center gap-2 py-12 text-gray-400"><p className="text-sm">No hay tecnologías</p></div>
                ) : (
                  <table className="w-full text-sm">
                    <thead className="border-b border-gray-100">
                      <tr>
                    ...
                    <tbody>
                      {tecnologias.map((t) => (
                        <tr key={t.id_tecnologia} className="border-b border-gray-50 hover:bg-gray-50 transition">
                          ...
                          <td className="px-4 py-3">
                            <div className="flex gap-2">
                              <button onClick={() => abrirEditarTec(t)}
          ...
          {tab === 'grados' && (
            <div className="space-y-5">
              <div className="flex items-start justify-between">
                ...
                <button onClick={abrirNuevoGrado}
                  className="rounded-full bg-blue-600 text-white px-4 py-2 text-sm font-medium hover:bg-blue-700 transition">
                  + Agregar grado
                </button>
              </div>
              <div className="rounded-2xl border border-app-border bg-white shadow-sm overflow-hidden">
                {loadingG ? <SkeletonRows n={4} /> : grados.length === 0 ? (
                  <div className="flex flex-col items-center gap-2 py-12 text-gray-400"><p className="text-sm">No hay grados</p></div>
                ) : (
                  <table className="w-full text-sm">
                    <thead className="border-b border-gray-100">
                      <tr>
                    ...
                    <tbody>
                      {grados.map((g) => (
                        <tr key={g.id_grado} className="border-b border-gray-50 hover:bg-gray-50 transition">
                          ...
                          <td className="px-4 py-3">
                            <div className="flex gap-2">
                              <button onClick={() => abrirEditarGrado(g)}
                                className="rounded-full border border-gray-200 px-3 py-1 text-xs hover:bg-gray-50 transition">Editar</button>
          ...
          {tab === 'anuncios' && (
            <div className="space-y-5">
              <div className="flex items-start justify-between">
                ...
                <button onClick={abrirNuevoAnuncio}
                  className="rounded-full bg-blue-600 text-white px-4 py-2 text-sm font-medium hover:bg-blue-700 transition">
                  + Agregar anuncio
                </button>
              </div>
                  ...
                  {anuncios.map((a) => (
                    <article key={a.id_anuncio} className="rounded-2xl border border-app-border bg-white shadow-sm overflow-hidden flex flex-col">
                      ...
                      <div className="flex gap-2 px-4 py-3 border-t border-gray-100">
                        <button onClick={() => abrirEditarAnuncio(a)}
      ...
      {modal?.tipo === 'suspender' && (
        <ModalBase onClose={() => setModal(null)}>
          ...
          <p className="text-sm text-gray-500 mb-4">La cuenta será suspendida de inmediato.</p>
      ...
      {modal?.tipo === 'importar' && (
        <ModalBase onClose={() => !importando && setModal(null)}>
          ...
          <p className="text-sm text-gray-500 mb-3">Se importará <strong>{modal.archivo.name}</strong>. Esta acción sobreescribirá todos los datos.</p>
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 text-xs text-amber-700 mb-5">Esta acción es <strong>irreversible</strong>.</div>
          <div className="flex gap-3">
          ...
          <div className="space-y-3 mb-5">
            <div>
              ...
              <input type="text"
                ...
                placeholder="Ej: React" value={formTec.nombre}
            ...
            <div>
              ...
              <input type="text"
                className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm outline-none focus:border-blue-400"
                placeholder="Ej: Frontend" value={formTec.categoria}
      ...
      {modal?.tipo === 'eliminar-tecnologia' && (
        <ModalBase onClose={() => setModal(null)}>
          ...
          <p className="text-sm text-gray-500 mb-5">¿Seguro que deseas eliminar <strong>{modal.item.nombre}</strong>? Se eliminarán sus asociaciones con proyectos.</p>
          <div className="flex gap-3">
            <button className="flex-1 border border-gray-200 rounded-full py-2 text-sm hover:bg-gray-50" onClick={() => setModal(null)} disabled={saving}>Cancelar</button>
      ...
      {(modal?.tipo === 'nuevo-grado' || modal?.tipo === 'editar-grado') && (
        <ModalBase onClose={() => setModal(null)}>
          ...
          <div className="mb-5">
            ...
            <input type="text"
              className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm outline-none focus:border-blue-400"
              placeholder="Ej: Licenciatura" value={formGrado.nombre_grado}
      ...
      {(modal?.tipo === 'nuevo-anuncio' || modal?.tipo === 'editar-anuncio') && (
        <ModalBase onClose={() => { setModal(null); resetFormAnuncio(); }}>
          <h3 className="text-base font-semibold text-gray-800 mb-4">
          ...
          <div className="space-y-3 mb-5">
            <div>
              ...
              <input type="text"
                className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm outline-none focus:border-blue-400"
                placeholder="Título del anuncio" value={formAnuncio.titulo}
                onChange={(e) => setFormAnuncio((p) => ({ ...p, titulo: e.target.value }))} />
            </div>
            <div>
              ...
              <textarea
                ...
                rows={3} placeholder="Descripción (opcional si sube imagen)"
                value={formAnuncio.descripcion}
                onChange={(e) => setFormAnuncio((p) => ({ ...p, descripcion: e.target.value }))} />
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1">URL de redirección *</label>
              <input type="text"
                className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm outline-none focus:border-blue-400"
                placeholder="https://..." value={formAnuncio.url_redireccion}
                onChange={(e) => setFormAnuncio((p) => ({ ...p, url_redireccion: e.target.value }))} />
            </div>
            <div>
      ...
      {modalContexto && (
        <ModalBase onClose={() => setModalContexto(null)} wide>
          <div className="flex items-center gap-3 pb-4 mb-4 border-b border-gray-100">
            <div className={`w-11 h-11 rounded-full flex items-center justify-center text-sm font-bold shrink-0 ${modalContexto.estado_actual === 'suspendido' ? 'bg-red-100 text-red-700' : 'bg-blue-100 text-blue-700'}`}>
            ...
            <span className={`ml-auto text-xs px-2.5 py-1 rounded-full font-medium ${modalContexto.rol === 'admin' ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-600'}`}>
          ...
          <div className="mb-4">
            ...
            <span className={`text-xs px-2.5 py-1 rounded-full font-medium ${modalContexto.estado_actual === 'activo' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
              ...
              <div className="space-y-2">
                {modalContexto.contexto.ip && (
                  <div className="flex items-center gap-2 bg-gray-50 rounded-xl px-3 py-2">
                    ...
                    <span className="text-xs text-gray-500 w-32 shrink-0">Dirección IP</span>
                    <span className="text-xs font-mono bg-white border border-gray-200 px-2 py-0.5 rounded-lg text-gray-700">{modalContexto.contexto.ip}</span>
                  </div>
                )}
                {modalContexto.contexto.dispositivo && (
                  <div className="flex items-start gap-2 bg-gray-50 rounded-xl px-3 py-2">
                    <span className="text-sm shrink-0">💻</span>
                    <span className="text-xs text-gray-500 w-32 shrink-0">Dispositivo</span>
                ...
                {modalContexto.contexto.tabla_principal_afectada && (
                  <div className="flex items-center gap-2 bg-gray-50 rounded-xl px-3 py-2">
                    ...
                    <span className="text-xs text-gray-500 w-32 shrink-0">Tabla afectada</span>
                    <span className="text-xs font-mono bg-blue-50 text-blue-700 border border-blue-100 px-2 py-0.5 rounded-lg">{modalContexto.contexto.tabla_principal_afectada}</span>
                  </div>
                )}
                {modalContexto.contexto.accion && (
                  <div className="flex items-center gap-2 bg-gray-50 rounded-xl px-3 py-2">
                    <span className="text-sm">⚡</span>
                    <span className="text-xs text-gray-500 w-32 shrink-0">Acción</span>
                ...
                {modalContexto.contexto.motivo && (
                  <div className="flex items-start gap-2 bg-amber-50 border border-amber-100 rounded-xl px-3 py-2">
                    <span className="text-sm shrink-0">📝</span>
                    <span className="text-xs text-gray-500 w-32 shrink-0">Motivo</span>
                ...
                {modalContexto.contexto.ejecutado_por && (
                  <div className="flex items-center gap-2 bg-gray-50 rounded-xl px-3 py-2">
                    <span className="text-sm">👤</span>
                    <span className="text-xs text-gray-500 w-32 shrink-0">Ejecutado por</span>
                ...
                {modalContexto.contexto.fecha_ult_acceso_anterior && (
                  <div className="flex items-center gap-2 bg-gray-50 rounded-xl px-3 py-2">
                    <span className="text-sm">🕐</span>
                    <span className="text-xs text-gray-500 w-32 shrink-0">Acceso anterior</span>
                ...
                {modalContexto.contexto.archivo && (
                  <div className="flex items-center gap-2 bg-gray-50 rounded-xl px-3 py-2">
                    <span className="text-sm">📁</span>
                    <span className="text-xs text-gray-500 w-32 shrink-0">Archivo</span>
          ...
          <button onClick={() => setModalContexto(null)}
      ...
      {toast && (
        <div className={`fixed bottom-6 right-6 z-50 flex items-center gap-3 px-4 py-3 rounded-2xl text-sm font-medium shadow-lg
